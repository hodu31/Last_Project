...
class Body:
    def __init__(self, score, xmin, ymin, xmax, ymax, keypoints_score, keypoints, keypoints_norm, track_id=None):
...
class MovenetMPOpenvino:
    def __init__(self, input_src=None,
        ...
        if input_src.endswith('.jpg') or input_src.endswith('.png') :
            ...
            if not self.cap.isOpened():
                print(f"Error: Cannot open video {input_src}")
                return
        ...
        if self.csv_output:
            ...
            fieldnames = ['NUMOFBODIES', 'ID', 'TIMESTAMP', 'FRAME_NUM']
            for name in KEYPOINT_DICT.keys():
                fieldnames.extend([f"{name.upper()}_X", f"{name.upper()}_Y"])
    ...
    def save_to_csv(self, bodies, csv_writer, frame_num):
        COLUMN_ORDER = [
        ...
        for body in bodies:
            ...
            if head_position and head_position[0] is not None and head_position[1] is not None:
                body.keypoints = np.append(body.keypoints, [head_position], axis=0)

                # Construct data as a dictionary
                csv_data = {'NUMOFBODIES': len(bodies)}

                for column in COLUMN_ORDER:
                    joint_index = KEYPOINT_DICT[column.lower()]
                    csv_data[f"{column.upper()}_X"] = body.keypoints[joint_index][0]
                    csv_data[f"{column.upper()}_Y"] = body.keypoints[joint_index][1]
...
if __name__ == "__main__":
    ...
    for src in file_name_lst:
        file_name = src[8:15] + "_" + "{}".format(num)
        csv_output = os.path.join(os.path.dirname(src), f"{os.path.basename(src).split('.')[0]}_{num}.csv")
        try:
            pd = MovenetMPOpenvino(input_src=src,
                        xml=args.xml,
                        # device=args.device, 
