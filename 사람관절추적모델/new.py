...
model1 = load_model('C:/Last_Project/사람관절추적모델/models/88모델.h5')
    ...
    def pd_render(self, frame, bodies):
        ...
        for body in bodies:
            ...
            if len(self.temp_array) >= 610 and self.frame_counter % 10 == 0:
                ...
                x = (body.xmin + body.xmax) // 2
                y = body.ymin
                cv2.putText(frame, f"pred:{predicted_label}", (x,y), cv2.FONT_HERSHEY_PLAIN, 4, color_box, 3)
    ...
    def save_to_array(self, bodies):
        ...
        for body in bodies:
            ...
            for name, index in KEYPOINT_DICT.items():
                data_row.extend([body.keypoints[index][0], body.keypoints[index][1]])
            data_row.append(self.current_value)  # Add the current value to the end of the row
            data_row.append(5)  # Add a new column with value 0
            data_rows.append(data_row)
        ...
        if len(data_rows) > 0:
            if len(self.temp_array) == 0:
                ...
                self.temp_array = np.vstack((self.temp_array, np.array(data_rows)))
