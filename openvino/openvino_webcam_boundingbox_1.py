...
model1 = load_model('C:/Last_Project/사람관절추적모델/models/smoke (2).h5')
...
### 웹캠에 표시하는 부분
    def pd_render(self, frame, bodies):
        ...
        for body in bodies:
            ...
            if len(self.temp_array) >= 610 and self.frame_counter % 10 == 0:
                prediction = model1.predict(np.array([self.temp_array]))
                self.predicted_label = np.argmax(prediction, axis=1)

            # Check if predicted_label is not None before drawing
            if self.predicted_label is not None:
                text_position_1 = (body.xmin, body.ymin+30)
                cv2.putText(frame, f"pred:{self.predicted_label[0]}", text_position_1, cv2.FONT_HERSHEY_PLAIN, 4, color_box, 3)
            
            if len(bodies) > 0:
                body = bodies[0]
                current_keypoints = body.keypoints
                if self.prev_keypoints is not None:
                    total_movement = np.sum(np.abs(current_keypoints - self.prev_keypoints))
                    if total_movement < 10 * 17:  # 각 관절당 5픽셀 내외로 움직임이 없다면 '멈춤'이라 판단
                        self.stop_frame_count += 1
                        if self.stop_frame_count >= 30:
                            # 정지된 시간을 계산합니다.
                            stopped_time = self.stop_frame_count / self.video_fps
                        ...
                        self.stop_frame_count = 0  # 움직임이 감지된 경우 프레임 카운트 재설정
    ...
    def save_to_array(self, bodies):
        ...
        for body in bodies:
            ...
            for name, index in KEYPOINT_DICT.items():
                data_row.extend([body.keypoints[index][0], body.keypoints[index][1]])
    ...
    def run(self):
        ...
        while True:
            ...
            if len(self.temp_array) > 610:
                self.temp_array = self.temp_array[1:]
