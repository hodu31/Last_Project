...
class MovenetMPOpenvino:
    def __init__(self, input_src=None,
        ...
        if self.csv_output:
            ...
            for name in KEYPOINT_DICT.keys():
                fieldnames.extend([f"{name.upper()}_X", f"{name.upper()}_Y", f"{name.upper()}_CONFIDENCE_LEVEL"])
    ...
    def run(self):
        ...
        frame_num = 0  # 프레임 번호를 추적하기 위한 변수를 추가합니다.
        ...
        while True:
            ...
            # CSV로 저장하기 위해 save_to_csv 함수를 호출합니다.
            self.save_to_csv(bodies, frame_num)

            frame_num += 1  # 프레임 번호를 증가시킵니다.
    ...
    def save_to_csv(self, bodies, frame_num):
        # 파일을 쓰기 모드로 열고 csv_writer 초기화
        with open(f'output_{frame_num}.csv', 'w', newline='') as csvfile:
            fieldnames = ['NUM_BODIES']
            COLUMN_ORDER = [
            ...
            csv_writer.writeheader()  # 헤더 행 쓰기
            ...
            for body in bodies:
                ...
                data_row = {'NUM_BODIES': len(bodies)}  # Start with the total number of bodies
                
                for name in COLUMN_ORDER:
                    index = KEYPOINT_DICT[name.lower()]
                    data_row[f"{name.upper()}_X"] = body.keypoints[index][0]
                    data_row[f"{name.upper()}_Y"] = body.keypoints[index][1]
                    data_row[f"{name.upper()}_CONFIDENCE_LEVEL"] = round(body.keypoints_score[index],2)
...
if __name__ == "__main__":
    ...
    pd = MovenetMPOpenvino(input_src=args.input, 
                    ...
                    output=args.output,
                    csv_output='C:/Users/de31/Documents/output1.csv')
    pd.run()
