...
### 웹캠에 표시하는 부분
    def pd_render(self, frame, bodies):
        ...
        for body in bodies:
            ...
            if body.track_id not in self.predicted_label and len(self.temp_array_dict[body.track_id]) >= 180:
...
################################################# 모델##############################################      
            ...
            # smoke
            if len(self.temp_array_dict[body.track_id]) >= 180 and self.frame_counter % 30 == 0:
    ...
    def save_to_array(self, bodies):
        ...
        for body in bodies:
            if body.track_id in self.temp_array_dict and len(self.temp_array_dict[body.track_id]) >= 180:
